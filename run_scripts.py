"""
Run the compatibility scripts provided by the user with the correct Excel file.
"""
import os
import sys
import shutil
...
def main():
    # Create a working copy of the product data Excel file
    source_file = os.path.join('data', 'Product Data.xlsx')
    target_file = os.path.join('data', 'test_base_doors.xlsx')
    
    if not os.path.exists(source_file):
        logger.error(f"Source Excel file not found: {source_file}")
        return
    
    # Make a copy for the scripts to work with
    shutil.copy2(source_file, target_file)
    logger.info(f"Created working copy of product data: {target_file}")
    
    try:
        # Get the current directory
        current_dir = os.path.dirname(os.path.abspath(__file__))
        
        # Create a modification to the script to run in our environment
        base_doors_script = os.path.join(current_dir, 'run_base_doors.py')
        with open(base_doors_script, 'w') as f:
            f.write("""
import os
import sys
import pandas as pd
...
""")
        ...
        # Create a folder for the original scripts
        os.makedirs('scripts', exist_ok=True)
        
        # Create the script file
        with open(os.path.join('scripts', 'base_doors_walls.py'), 'w') as f:
            f.write("""
import pandas as pd
import os
...
""")
        ...
        # Run the scripts
        logger.info("Running the base doors compatibility script...")
        os.system(f"{sys.executable} {base_doors_script}")
        
        # After running the script, copy the compatibility data to the database
        import_script = os.path.join(current_dir, 'import_results.py')
...
# Load the bases sheet which now has compatibility data
try:
    bases_df = excel_file.parse('Shower Bases')
    print(f"Loaded {len(bases_df)} shower bases with compatibility data")
except Exception as e:
    print(f"Error loading Shower Bases: {e}")
    exit(1)

# Create database connection
engine = create_engine(DATABASE_URL)

# First clear any existing compatibility data
with engine.connect() as conn:
    conn.execute(text("TRUNCATE TABLE compatibilities"))
    conn.commit()
    print("Cleared existing compatibility data")

# Process each base
compatibility_count = 0
for _, base in bases_df.iterrows():
    base_sku = base.get('Unique ID') or base.get('SKU')
    if not base_sku or pd.isna(base_sku):
        continue
        
    # Process compatible doors
    compat_doors = base.get('Compatible Doors')
    if compat_doors and not pd.isna(compat_doors):
        doors = str(compat_doors).split('|')
        for door in doors:
            door = door.strip()
            if not door:
                continue
            ...
            try:
                ...
                print(f"Error inserting door compatibility: {e}")
    
    # Process compatible walls
    compat_walls = base.get('Compatible Walls')
    if compat_walls and not pd.isna(compat_walls):
        walls = str(compat_walls).split('|')
        for wall in walls:
            wall = wall.strip()
            if not wall:
                continue
            ...
            try:
                ...
                print(f"Error inserting wall compatibility: {e}")

print(f"Imported {compatibility_count} compatibility relationships")
""")
        
        logger.info("Running the import results script...")
        ...
        if os.path.exists(target_file):
            logger.info(f"Keeping working file for reference: {target_file}")
